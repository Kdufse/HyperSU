...
#
# boot_patch.sh      script        A script to patch boot image for HyperSU.
...
echo "****************************"
echo " HyperSU Boot Image Patcher"
echo "****************************"
...
if [ ! $(./kptools -i kernel -f | grep CONFIG_KALLSYMS=y) ]; then
	...
	echo "- HyperSU requires CONFIG_KALLSYMS to be Enabled."
...
if [ ! $(./kptools -i kernel.ori -f | grep CONFIG_KALLSYMS_ALL=y) ]; then
	...
	echo "- HyperSU has patched but maybe your device won't boot."
