...
## What is HyperSU?
HyperSU is a root solution similar to Magisk or KernelSU that unites the best of both.
...
## What's the difference between HyperSU and Magisk?
- Magisk modifies the init system with a patch in your boot image's ramdisk, while HyperSU patches the kernel directly.


## HyperSU vs KernelSU
- KernelSU requires the source code for your device's kernel which is not always provided by the OEM. HyperSU works with just your stock `boot.img`.


## HyperSU vs Magisk, KernelSU
- HyperSU allows you to optionally not modify SELinux, this means that the APP thread can be rooted, libsu and IPC are not necessary.
...
## Relationship between HyperSU and KernelPatch

HyperSU depends on KernelPatch, inherits all its capabilities, and has been expanded.
...
- In addition, HyperSU directly utilizes magiskpolicy to provide additional SELinux support.
