...
## Що таке HyperSU?
HyperSU - це root рішення, схоже на Magisk або KernelSU, яке поєднує найкраще з обох.
Воно поєднує зручний і простий метод установки Magisk через boot.img з потужними можливостями виправлення ядра KernelSU.

## У чому різниця між HyperSU та Magisk?
- Magisk змінює систему ініціалізації за допомогою патча на RAM диску вашого завантажувального образу, в той час як HyperSU вносить зміни безпосередньо в ядро.


## HyperSU проти KernelSU
- Для KernelSU потрібен вихідний код ядра вашого пристрою, який не завжди надається OEM-виробником. HyperSU працює безпосередньо з вашим вихідним `boot.img`.


## HyperSU проти Magisk, KernelSU
- HyperSU дозволяє вам за бажання не змінювати SELinux. Це означає, що потік програми може бути рутований, в libsu і IPC немає необхідності.
...
## Зв'язок між HyperSU та KernelPatch

HyperSU заснований на KernelPatch, успадкував усі його можливості та був розширений.
...
- KernelPatch не змінює контекст SELinux та обходить SELinux за допомогою перехоплення.
  ...
  Це дуже зручно.
- Крім того, HyperSU безпосередньо використовує magiskpolicy для забезпечення додаткової підтримки SELinux.
