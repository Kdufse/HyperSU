...
HyperSU是一种类似于Magisk或KernelSU的root解决方案，但HyperSU提供更多功能。
HyperSU分别结合了Magisk方便易用的通过`boot.img`安装的方法，和KernelSU强大的内核修补能力。

## HyperSU与Magisk的区别？

- Magisk对启动映像中的ramdisk进行补丁，以修改init系统。而HyperSU则直接修补Linux内核。

## HyperSU与KernelSU的区别？

- KernelSU需要您设备的内核的源代码，而OEM并不总是提供该源码。而HyperSU仅需要您的设备原本的`boot.img`。

## HyperSU与Magisk、KernelSU的区别？

- HyperSU可选择不修改SELinux，这意味着Android应用程序线程可以被root，无需libsu和IPC。
- HyperSU提供**Kernel Patch Module（KP模块）**。
...
## HyperSU与KernelPatch的关系

HyperSU依赖于KernelPatch，继承了其所有功能并进行了扩展。
...
- 此外，HyperSU直接利用magiskpolicy提供额外的SELinux支持。  
